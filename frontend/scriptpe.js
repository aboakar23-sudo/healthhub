...
function openBookingModal(docId) {
    ...
    // إنشاء قائمة الأوقات بفاصل 15 دقيقة
    ...
    try {
        ...
        Array.from(timeSelect.options).forEach(option => {
            if (option.value) {
                ...
                option.textContent = isBooked ? `${option.value} (محجوز)` : option.value;
            }
        });
...
async function toggleDoctorInfo(docId) {
    ...
    try {
        ...
        // صور الشهادات (محفوظة في localStorage بـ certImages_{docId})
        const savedCerts = localStorage.getItem(`certImages_${docId}`);
        if (savedCerts) {
            try {
